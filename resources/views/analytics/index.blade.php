...
<div class="mb-8 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
    <div>
        ...
        <p class="admin-page-subtitle">Metrics and performance insights</p>
    </div>
</div>
...
<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
    <div class="admin-card-hover p-5">
        <p class="text-xs font-medium text-ink/50 uppercase">Messages sent</p>
        <p class="admin-stat-value">{{ number_format($kpis['messages_sent']) }}</p>
    </div>
    <div class="admin-card-hover p-5">
    ...
    <div class="admin-card-hover p-5">
        ...
        <p class="mt-1 text-2xl font-bold text-coral">{{ $kpis['rsvp_rate'] }}%</p>
    </div>
    <div class="admin-card-hover p-5">
    ...
    <div class="admin-card-hover p-5">
        <p class="text-xs font-medium text-ink/50 uppercase">Social engagement</p>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="admin-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-4">Messages by channel (bar)</h2>
        <div class="h-64">
            <canvas id="bar-chart"></canvas>
        </div>
    </div>
    <div class="admin-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-4">Messages sent (last 7 days)</h2>
        <div class="h-64">
            <canvas id="line-chart"></canvas>
        </div>
    </div>
    <div class="admin-card p-6">
        <h2 class="text-lg font-semibold text-ink mb-4">Engagement (pie)</h2>
...
<div class="admin-table-wrap">
    <h2 class="px-6 py-4 text-lg font-semibold text-ink bg-dawn-2/30 border-b border-dawn-2/80">Event performance</h2>
    <table class="admin-table">
        <thead>
            <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink/50">RSVP rate</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink/50">Attendance %</th>
                <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-ink/50">Messages</th>
                <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-ink/50">Report</th>
            </tr>
        </thead>
        <tbody>
            @forelse($eventPerformance as $perf)
            <tr>
                <td class="px-6 py-4 font-medium text-ink">{{ $perf['event']->name }}</td>
                <td class="px-6 py-4 text-sm text-ink/65">{{ $perf['attendees'] }}</td>
                <td class="px-6 py-4 text-sm text-ink/65">{{ $perf['rsvp_rate'] }}%</td>
                <td class="px-6 py-4 text-sm text-ink/65">{{ $perf['attendance_rate'] }}%</td>
                <td class="px-6 py-4 text-sm text-ink/65">{{ $perf['messages'] }}</td>
                <td class="px-6 py-4 text-right">
                    <a href="{{ route('analytics.event-report', $perf['event']) }}" class="admin-link text-sm">View report</a>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="6" class="px-6 py-12 text-center text-ink/50">No events yet</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
@php
    ...
    $barColors = $chartData['bar']['colors'] ?? ['#E8604C', '#10b981', '#f59e0b', '#ec4899'];
